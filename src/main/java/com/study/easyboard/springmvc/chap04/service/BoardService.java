...
import com.study.easyboard.springmvc.chap04.mapper.PostMapper;
...
public class BoardService {
    ...
    public void createBoardForUser(String account) {
        Board board = Board.builder()
                .account(account)
                .title(account + "'s Board")
                .build();
        boardMapper.save(board);
    }

    public Board findBoardByAccount(String account) {
        return boardMapper.findByAccount(account);
    }

    public List<Post> findPostsByBoardNo(int boardNo) {

        return postMapper.findByBoardNo(boardNo);
    }
